        ...
        <div className="flex gap-6 h-full">
          ...
          <aside className={`${showSidebar ? "w-80" : "w-0"} hidden md:block transition-all duration-200 overflow-hidden`}> 
            <div className="flex flex-col gap-4">
              <Button onClick={newChat} className="w-full flex items-center gap-2 justify-center">
                    ...
                    {history.length === 0 ? (
                      ...
                      history.map((h) => (
                        <button
                          ...
                          className={`text-left p-2 rounded-md hover:bg-accent/10 dark:hover:bg-accent/20 w-full ${selected === h.id ? "ring-2 ring-accent/50" : ""}`}
                        >
                ...
                <div className="flex flex-col gap-4">
                  {messages.map((m) => (
                    <div key={m.id} className={`max-w-[85%] ${m.role === "user" ? "ml-auto text-right" : "mr-auto text-left"}`}>
                      <div className={`inline-block rounded-2xl p-4 text-lg leading-relaxed shadow-lg ${m.role === "user" ? "bg-primary text-primary-foreground dark:shadow-[0_10px_30px_rgba(124,58,237,0.28)]" : "bg-popover text-popover-foreground dark:shadow-[0_6px_18px_rgba(2,6,23,0.6)]"} whitespace-pre-wrap`}>
                        {m.isCode ? (
                          <pre className="bg-transparent text-sm overflow-auto rounded">
                            <code>{m.text}</code>
                          </pre>
                        ) : (
                          <div>{m.text}</div>
